...
M.launch=function()
{
	...
	M.draw=function()
	{
		...
		if (Game.drawT%10==0)
		{
			...
			if (M.parent.level<M.plotLimits.length) l('gardenPlotSize').innerHTML='<small>Plot size : '+Math.max(1,Math.min(M.plotLimits.length,M.parent.level))+'https://cdn.jsdelivr.net/gh/Haha64142/seraph@main/'+M.plotLimits.length+'<br>(Upgrades with farm level)</small>';
			else l('gardenPlotSize').innerHTML='';
			l('gardenSeedsUnlocked').innerHTML='Seeds<small> ('+M.plantsUnlockedN+'https://cdn.jsdelivr.net/gh/Haha64142/seraph@main/'+M.plantsN+')</small>';
			for (var i in M.soils)
			{
				var me=M.soils[i];
